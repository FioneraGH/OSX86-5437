 ...
 *
 * Disassembly of SSDT-6.aml, Sun Dec 31 20:51:27 2017
 *
...
DefinitionBlock ("", "SSDT", 1, "OptRef", "OptTabl", 0x00001000)
{
    ...
    Scope (\_SB.PCI0.RP05.PEGP)
    {
        ...
        {
            /* patched remove \_SB.PCI0.LPCB.EC.ECNV (0x02) */
            If (LNotEqual (GPRF, One))
            {
                If (LEqual (CTXT, Zero))
                {
    ...
    Scope (\_SB.PCI0.RP05.PEGP)
    {
        ...
        {
            If (LEqual (\_SB.PCI0.RP05.PEGP.SGPI (PWOK), One))
            {
                Store ("<<< GETS() return 0x1 >>>", Debug)
                Return (One)
            }
            ElseIf (LEqual (\_SB.PCI0.LPCB.EC.FBST (), One))
            {
